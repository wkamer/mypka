...
**How to apply:** For every build or execution task, first show a concrete plan (what will be done, in which steps, with what result) and explicitly wait for "yes, proceed" before making the first tool call. This also applies to scripts, API calls, n8n workflows, and file writes. "Reading files to understand the pattern" does NOT count as permission to proceed — the plan still comes first, even after exploration.

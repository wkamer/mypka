# Lifecycle Sweep Model Design v01

**Document type:** Proposal — design artifact
**Status:** Active
**Session:** 2026-06-15
**Author:** Larry (Team Orchestrator)
**Predecessor:** Chat-only design session 2026-06-15 — Owner authorized this persisted version
...
| Stalled at Triaged | `triaged` | irrelevant | NULL | threshold — **see OD-1** | NULL |
...
| Carry-Forward | `active` or `ready` | NULL or unknown | NULL | See OD-2 — no confirmed structured value yet |
...
| MAC-LC-4 | Stalled at Triaged is inactive and shown as `[THRESHOLD NOT DEFINED — INACTIVE]` until OD-1 is resolved |
...
| MAC-DL-2 | Carry-Forward shown as `[REQUIRES VERIFICATION — OD-2 OPEN]` until OD-2 is resolved |
...
These decisions must be made before the respective categories can be implemented.

| Code | Decision | Blocks |
|---|---|---|
| OD-1 | **Stalled-at-Triaged threshold (LC sweep):** How many days may a `triaged` LC remain without `processed_at` before it is classified as Stalled? GL-022 does not define this. | MAC-LC-4; Stalled at Triaged category |
| OD-2 | **Carry-Forward structure (DL sweep):** Is Carry-Forward a structured `owner_decision` value, an annotation in `processing_notes`, or a separate `state` value? Without this, Carry-Forward cannot be auto-classified. | MAC-DL-2; Carry-Forward category |

**Previously resolved in this session (documented for reference):**
...
*Delivered at: Deliverables/20260615_Core_Lifecycle Sweep Model Design/lifecycle-sweep-model-design-v01.md*

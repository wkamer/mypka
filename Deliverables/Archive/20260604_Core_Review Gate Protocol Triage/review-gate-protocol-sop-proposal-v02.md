# Review Gate Protocol — SOP Proposal v02

**File:** `Deliverables/20260604_Core_Review Gate Protocol Triage/review-gate-protocol-sop-proposal-v02.md`
**Date:** 2026-06-04
**Prepared by:** Larry, Team Orchestrator
**Status:** Formal proposal v02 — awaiting Owner decision
**Supersedes:** `review-gate-protocol-sop-proposal-v01.md`
**Paired with:** `review-gate-protocol-gl-proposal-v02.md`
...
| # | Change | Location |
|---|---|---|
| 1 | Added `**Owner:** Walter Kamer` directly before `**Maintainer:** Larry, Team Orchestrator` in the exact SOP-016 content | Section 4 — exact SOP content header |
| 2 | Replaced Owner decision options with a safer set. SOP-016-only implementation while GL-016 does not exist on disk is no longer presented as a normal executable option. It is retained as Option A but explicitly marked invalid in the current state. | Section 10 — Owner Decision Options |
| 3 | Updated Section 8 (Relationship to the Paired GL Proposal) to reflect the revised option set | Section 8 |
...
| Paired GL | GL-016 — Review Gate for Governance-Relevant Deliverables (see `review-gate-protocol-gl-proposal-v02.md`) |
| Execution order | GL-016 must exist on disk and all GL-016 post-checks must pass before SOP-016 implementation begins. If GL-016 implementation fails, SOP-016 implementation must not start. |
...
1. Confirm that `Team Knowledge/Core/Guidelines/GL-016_Review Gate for Governance-Relevant Deliverables.md` exists on disk and all GL-016 post-checks have passed. If this confirmation fails, halt and surface to Owner before proceeding.
2. Create the file `Team Knowledge/Core/SOPs/SOP-016_Review Gate Procedure for Governance-Relevant Deliverables.md` with the exact content from Section 4.
3. Add one row to `Team Knowledge/Core/SOPs/SOP-index.md` as specified in Section 5.
...
- No GL-016 file is created by this implementation (separate prior execution required)
...
|---|---|---|
| 1 | GL-016 prerequisite | `Team Knowledge/Core/Guidelines/GL-016_Review Gate for Governance-Relevant Deliverables.md` confirmed present on disk before SOP-016 implementation begins. If absent: halt and surface to Owner. |
...
| 5 | Owner field present | File contains `**Owner:** Walter Kamer` before `**Maintainer:**` |
| 6 | SOP-index.md updated | The SOP-016 row from Section 5 is present in `SOP-index.md` |
| 7 | No other files modified | No other file in the vault was touched during implementation |
| 8 | No database writes | No team_tasks, team_log, UMC, or memory-db entries were written |
...
passed. This is a hard prerequisite, not a recommendation. If GL-016 implementation fails or
is deferred, SOP-016 implementation is blocked. The executing agent halts and surfaces the
blocker to Owner if GL-016 is not confirmed present.

---

## 8. Relationship to the Paired GL Proposal

This SOP proposal and `review-gate-protocol-gl-proposal-v02.md` form a paired set.

- GL-016 establishes the principle this SOP implements.
- If Owner approves Option B (paired implementation), GL-016 is implemented first, post-checks
  pass, then SOP-016 implementation begins.
- If Owner approves Option A in the GL proposal (GL-016 only), this SOP remains a pending
  proposal and may be submitted for separate approval in a later session once GL-016 is
  confirmed on disk.
- If Owner approves Option A in this SOP proposal (SOP-016 only) while GL-016 does not exist
  on disk: this option is not valid in the current state. See Section 10.
...
| GL-016 is not yet on disk when SOP-016 implementation is attempted | Post-check 1 is a hard halt: if GL-016 is absent, the executing agent must stop and surface to Owner before taking any further action |
| Mode 3 fallback is used as the default mode instead of as a fallback | Mode 3 must be explicitly declared. The declaring agent must name which modes were unavailable and why. Mode 1 or Mode 2 is always preferred. |
| Worked examples become stale as the operating model evolves | Examples are illustrative. The 13 checks and hard stop conditions are the authoritative reference. New deliverable types require a scope table update in GL-016, not an update to the worked examples here. |
...
| Option | Action |
|---|---|
| **A** | **Currently not valid.** Approve SOP-016 implementation only — this option becomes valid only after GL-016 exists on disk and all GL-016 post-checks have passed. While GL-016 does not exist, selecting Option A would produce a SOP that references a non-existent GL. |
| **B** | Approve paired implementation — GL-016 first, post-checks pass, then SOP-016 **(recommended)** |
| **C** | Request amendments to this SOP proposal — specify which sections require changes; a v03 will be prepared |
| **D** | Defer both proposals |
| **E** | Reject |

**Note on Option A:** It is preserved in this option set so that it can be selected in a future
session where GL-016 already exists on disk and SOP-016 is being considered independently. In
the current state (GL-016 not yet on disk), Option A must not be executed.

---

*Delivered on: 2026-06-04*
*Delivered at: Deliverables/20260604_Core_Review Gate Protocol Triage/review-gate-protocol-sop-proposal-v02.md*

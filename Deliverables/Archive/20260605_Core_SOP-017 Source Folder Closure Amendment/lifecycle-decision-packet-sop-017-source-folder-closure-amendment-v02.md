...
This packet proposes lifecycle decisions for all 11 deliverables in the source folder and provides the source folder closure plan required by the now-live SOP-017 Section 10 and EX-8.

---

## Source Folder Contents (11 files)
...
**Recommendation:** Archive. v01 was superseded when v02 was accepted and implemented. No standalone authoritative value. No knowledge extraction required. Archive to the domain Archive subfolder.
...
| Risk level | Low |
| Decision rules applied | R1 (Authoritative marker), R4 (Governance reference), R8 (Archive eligible — does not carry protection against archiving since Authoritative marker is recorded in execution report, not a block on archiving) |

**Recommendation:** Assign Authoritative marker (record in execution report), then archive. The amendment text is preserved in SOP-017 itself; v02 is the authoritative source for the proposal-as-submitted record. Archiving does not delete it; it moves it to the Archive subfolder where it remains retrievable.
...
**Recommendation:** Assign Authoritative marker (record in execution report), then archive. Before archiving, confirm the future graduation candidate is preserved per the plan in the section below. The archived DP-5 record remains the durable source; the lifecycle execution report serves as the forward-reference governance record for the candidate.
...
The DP-5 record is moved to `Deliverables/Archive/`. It remains retrievable. The lifecycle execution report in the new active folder (see Source Folder Closure Plan below) holds the Indexed marker and the forward reference. The candidate is not lost.
...
1. After all 11 authorized archive moves have been executed (artifacts 1 through 11 moved to `Deliverables/Archive/20260605_Core_SOP-017 Source Folder Closure Amendment/`), the Maintainer checks the source folder using a hidden-file-aware method:
...
**Recommendation:** Write the lifecycle execution report to a NEW, SEPARATE deliverables folder, not inside the source folder.

**Proposed path for lifecycle execution report folder:**
`Deliverables/20260605_Core_SOP-017 Amendment Lifecycle Execution/`

**Rationale:** If the lifecycle execution report were placed inside the source folder (`Deliverables/20260605_Core_SOP-017 Source Folder Closure Amendment/`), it would remain in the folder after all 11 artifacts are archived, preventing the source folder from being confirmed empty. The source folder check would then surface the execution report as a remaining file, creating a circular dependency: the execution report records the source folder status, but its presence prevents the source folder from being empty.

Writing the execution report to a separate folder avoids this problem entirely. After all 11 artifacts are archived, the source folder is checked and expected to be empty. The execution report exists in its own deliverables folder outside the archive scope.

**Uncertainty:** The proposed folder name `20260605_Core_SOP-017 Amendment Lifecycle Execution` follows the `YYYYMMDD_Domain_beschrijving` convention. If the Owner prefers a different descriptive suffix, this should be confirmed before execution begins. The important constraint is that the folder must be outside `Deliverables/20260605_Core_SOP-017 Source Folder Closure Amendment/`.

**Safest Owner decision:** Confirm the separate execution report folder path at DP-6 before any archive moves begin, so the execution report destination is fixed before the source folder closure check runs.

---

## Summary of Proposed Archive Scope

All 11 artifacts are proposed for archiving to:
...
**Artifacts with Authoritative marker to assign before archiving:** 2, 5, 7, 8, 9, 10, 11 (7 artifacts)

**All 11 artifacts require a formal archive proposal per EX-6 before any file is moved.**
...
|---|---|
| Archive all 11 artifacts as proposed | Yes / No / amend |
| Authoritative marker for artifacts 2, 5, 7, 8, 9, 10, 11 | Confirm per artifact |

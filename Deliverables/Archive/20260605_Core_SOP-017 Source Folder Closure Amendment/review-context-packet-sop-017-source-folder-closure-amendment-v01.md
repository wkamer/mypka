...
| SOP-018 — Idea-to-Implementation Routing Procedure | Governs scenario classification (S8), route selection (Route D), and review gate triggers for this amendment. |
...
After the reviewer returns findings, the Owner may choose from the following options (per SOP-016 Section 7):

| Option | Meaning |
|---|---|
| Approve execution | Amendment accepted as stated; implementation may proceed per SOP-015 Step 6. |
| Request amendments | Specific changes required before re-review; a v02 is prepared. |
...
| `Team Knowledge/Core/SOPs/SOP-018_Idea-to-Implementation Routing Procedure.md` | File — governs classification | Reviewer must verify S8 classification and Route D selection. |
...
No implementation may occur as a result of this review. This review produces findings and an Owner decision packet only. Implementation requires a separate Owner approval (DP-3, then DP-4) following the review findings. No file may be created, modified, moved, or deleted. No database writes may occur.
...
- The Review Context Packet (this document, all 18 fields complete)

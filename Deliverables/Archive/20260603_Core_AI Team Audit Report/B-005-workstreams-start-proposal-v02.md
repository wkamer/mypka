# B-005 Workstreams Start Proposal v0.2
...
**No changes executed**
**Supersedes:** B-005 v0.1 (2026-06-03)
...
**Owner terminology note:** Throughout this proposal and in all proposed Workstream file content, `Owner` refers exclusively to Owner Walter Kamer. Agent responsibility within Workstream steps is expressed as `**Step lead:**`, never `**Owner:**`.
...
- The GL-004 routing rule addition is low risk — one-line addition to a non-executable reference document.

**Mitigation:** exact content is provided below for Owner review before approval. After execution, Kai reads back all files and confirms no contradictions with Penn AGENT.md.
...
For image input:
1. Save file to `PKM/Images/YYYY/MM/YYYYMMDD_beschrijving.ext`
2. Embed in journal entry: `![[Images/YYYY/MM/YYYYMMDD_beschrijving.ext]]`
...
| **Risk** | Low — one-line addition + missing Changelog section |
| **Owner approval required** | Yes — GL modification |

**Exact insertion 1 — routing rule (insert after the `**Regel:**` note about Team Inbox, before the `**Scaffold-afwijking:**` note):**
...
**Exact insertion 2 — Changelog section (append at end of file, after the Pad-wijziging protocol section):**
...
| C-004: GL-004 routing rule + Changelog | Low — one addition + missing section added | Core vs Domain ambiguity; GL-004 non-compliant with GL-014 §5 |
...
2. Owner reviews exact content before approval
3. Read-back verification after execution confirms no AGENT.md contradictions
...
**Phase B-005A — Core Workstreams infrastructure (recommended to approve as next step):**
...
The Core Workstreams layer is the only static knowledge layer not yet activated. The highest-urgency gap is Penn's broken workflow contract: Penn's AGENT.md instructs Penn to read `WS-001_Daily journaling.md` before every journaling task and designates it as authoritative over AGENT.md. The file does not exist.

This proposal now provides exact content for every B-005A change. B-005A is documentation-only, the WS-001 content is derived from Penn's current AGENT.md to avoid unintended behavioral changes, and all post-checks are defined.

**Recommended next step:** Owner Walter Kamer reviews the exact WS-001 content in §9, confirms it matches expected Penn behavior, and approves B-005A for execution.
...
*Delivered at: `Deliverables/20260603_Core_AI Team Audit Report/B-005-workstreams-start-proposal-v02.md`*

# B-021C-A Secure Credential Recovery SOP-001 Proposal v0.4

**Status:** Proposal only — no implementation  
**Version:** v0.4 (supersedes v0.3)  
**Date:** 2026-06-03  
...
v0.3 replaced the existing Step 12c in full with a complete English credential recovery procedure but contained an ambiguous prohibition: "Do not print, paste, echo, `cat`, `grep`, or log secret values at any point." This wording inadvertently prohibits pasting credential values into the local `nano` editor — a required and expected action during manual recovery.

v0.4 corrects that wording, updates the Secret-Free Confirmation section accordingly, and updates the execution plan to use exact text matching instead of line numbers (which shift after Change A is applied).
...
| Location | Line(s) at time of read |
|---|---|
...
Note: line numbers are provided for reference only. Execution uses exact text matching (see §10) because Change A will shift subsequent line numbers.
...
5. No explicit prohibition on printing secrets outside the local editor
...
### 5.3 Why v0.3 Wording Was Corrected

v0.3 contained: "Do not print, paste, echo, `cat`, `grep`, or log secret values at any point."

The word "paste" in this context is ambiguous: it could be read as prohibiting the Maintainer from pasting credential values from Bitwarden into the local `nano` editor. That is a required step during manual recovery. The prohibition must target exposure outside the local editor, not the editor itself.

### 5.4 Correct Approach

Replace Step 12c in full with corrected wording that explicitly permits local editor entry while prohibiting all other forms of secret exposure.
...
**Reason:** Replaces a brief Dutch two-liner with a complete, English, self-contained recovery procedure. Eliminates the language violation, adds the permission gate, clarifies that local editor entry of credentials is permitted while all other exposure is prohibited.
...
- Permits the Maintainer to enter or paste credential values into the local `nano` editor — this is the required and only allowed point of secret entry
- Prohibits exposure of secret values in all other contexts: terminal commands, chat, documents, session logs, team_log, SOPs, Guidelines, Workstreams, and deliverables
- Instructs `stat` for permission verification — reads file metadata only, never file contents
...
The proposed text restricts credential entry to the local `nano` editor session only. `nano` writes to the file directly and does not output credential values to the shell, terminal history, or any log. The explicit prohibition list covers all other exposure vectors: terminal commands, chat, documents, session logs, team_log, SOPs, Guidelines, Workstreams, and deliverables.
...
**After Owner Walter Kamer gives explicit approval for B-021C-A v0.4:**

| Step | Action | Who |
|---|---|---|
| 1 | Locate Change A target: find the block beginning with the exact string `**12c — .env aanmaken:**` in `SOP-001_Disaster recovery.md` | Larry or Kai |
| 2 | Replace that entire block with the Change A replacement text (§6, Change A) using exact text match — do not rely on line numbers | Larry or Kai |
| 3 | Locate Change B target: find the line beginning with the exact string `In a DR scenario, credentials must be recovered manually from a secure source. Detailed recovery procedure: pending B-021C` | Larry or Kai |
| 4 | Replace that line with the Change B replacement text (§6, Change B) using exact text match — do not rely on line numbers, as they will have shifted after Change A | Larry or Kai |
| 5 | Locate `## Changelog` at the bottom of the file and append the Change C entry (§6, Change C) | Larry or Kai |
| 6 | Read back the modified Step 12c — confirm: no Dutch text, no secret values, local editor entry wording correct, permission gate present | Larry |
| 7 | Read back the modified Backup Infrastructure placeholder — confirm: cross-reference to Step 12c present, no secret values | Larry |
| 8 | Write execution report for B-021C-A | Larry |
| 9 | Mark B-021C complete in team_tasks (both A and B scopes done) | Larry |
...
1. **Approve Change A (Step 12c replacement):** Does the proposed replacement text accurately and completely describe the intended recovery procedure, including the corrected local-editor-only wording? Any corrections before approval?
...
**No implementation may happen until Owner Walter Kamer explicitly approves B-021C-A v0.4.**

Approval of v0.3 does not cover v0.4 — the proposed Step 12c text has changed. Approval must name B-021C-A v0.4 explicitly.
...
Approve Changes A, B, and C together. All three are required to fully close B-021C-A: a partial approval (e.g., only Change A without Change B) would leave the Backup Infrastructure placeholder pointing to a non-existent pending procedure.

Recommended sequence if approved: execute A → B → C in a single edit session using exact text matching, then read back both changed sections, then write the execution report and mark B-021C complete.

---

*Delivered on: 2026-06-03*  
*Delivered at: Deliverables/20260603_Core_AI Team Audit Report/B-021C-A-secure-credential-recovery-sop-proposal-v04.md*

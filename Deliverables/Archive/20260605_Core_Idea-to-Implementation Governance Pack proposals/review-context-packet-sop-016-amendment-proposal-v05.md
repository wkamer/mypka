...
1. **Section 11.2 clarified — system-file change terminology** — The bullet point "A proposal for a system-file change (SOP-015 deliverable)" now carries a parenthetical clarification stating that SOP-015 uses the term "system-file change" to refer to governance file changes, and cross-referencing SOP-018 Section 2.4 for the governance file / operational system file distinction when SOP-018 is active. This prevents conflict with SOP-018's more precise file category terminology while retaining SOP-015's own established wording.
2. **Section 11.7 companion file paths updated** — All four companion document paths updated from v03 to v04 versions.
3. **Acceptance criterion 6 updated** — Now verifies v04 paths in Section 11.7.
4. **Acceptance criterion 14 added** — Verifies that Section 11.2 uses the parenthetical clarification and does not conflict with SOP-018's governance file / operational system file distinction.
...
1. **Section 11.2 first bullet corrected — Route B without escalation exemption** — The v04 text "A proposal authorizing implementation (from any route per the active idea routing procedure, if one is in effect)" could be read as requiring an RCP for Route B without escalation. Route B without escalation is intentionally lightweight per SOP-018 v05: it does not require Review Gate, and it does not require an RCP unless the Owner explicitly requests one. The corrected text reads: "An implementation-authorizing proposal from any route that requires Review Gate under the active idea routing procedure. Route B without escalation does not require an RCP unless the Owner explicitly requests one." This is internally consistent with SOP-018 v05 Section 9 (Route B: no Review Gate without escalation), Section 12 (Route B without escalation: Owner reviews lightweight proposal directly; no Review Gate package), and Section 13 (Maintainer must confirm all Route B escalation conditions false before treating Route B as non-Review-Gate).
2. **Section 11.7 companion file paths updated** — All four companion document paths updated from v04 to v05 versions.
3. **Acceptance criterion 6 updated** — Now verifies v05 paths in Section 11.7.
4. **Acceptance criterion 15 added** — Verifies that Section 11.2 correctly exempts Route B without escalation from the RCP requirement.
...
6. Section 11.7 lists all four governance-pack companion documents with their current v05 paths and pending status.
...
14. Section 11.2 uses the parenthetical clarification for the system-file change bullet (noting SOP-015's terminology and cross-referencing SOP-018 Section 2.4). The Section 11.2 wording does not conflict with SOP-018's governance file / operational system file distinction.
15. Section 11.2 first bullet correctly limits the RCP requirement to routes that require Review Gate under the active idea routing procedure. The Route B without escalation exemption is explicitly stated: Route B without escalation does not require an RCP unless the Owner explicitly requests one. This is internally consistent with SOP-018 v05 Section 9 (Route B: Review Gate not required without escalation), Section 12 (Route B without escalation: Owner reviews lightweight proposal directly; no Review Gate package), and Section 13 (Maintainer must confirm all Route B escalation conditions false before treating Route B as non-Review-Gate). The exemption does not apply when any Route B escalation condition is true — in that case the route escalates and the rules of the escalated route apply.
...
| Request amendments | Specific changes required; revised v06 prepared; SOP-016 not modified until approved |
| Approve with modifications | Owner states exact modifications; v06 created before SOP-016 is modified |
...
| 2026-06-05 | v04 | Section 11.2: "system-file change" bullet clarified with parenthetical (SOP-015 uses "system-file change" for governance file changes; cross-reference to SOP-018 Section 2.4 for file category distinction). Section 11.7: companion file paths updated to v04 versions. Acceptance criterion 6 updated (v04 paths); acceptance criterion 14 added (Section 11.2 terminology does not conflict with SOP-018 file category distinction). |
| 2026-06-05 | v05 | Section 11.2 first bullet corrected: "A proposal authorizing implementation (from any route per the active idea routing procedure, if one is in effect)" replaced with "An implementation-authorizing proposal from any route that requires Review Gate under the active idea routing procedure. Route B without escalation does not require an RCP unless the Owner explicitly requests one." This aligns Section 11.2 with SOP-018 v05 Section 9 (no Review Gate for un-escalated Route B), Section 12 (Route B without escalation: direct Owner review, no Review Gate package), and Section 13 (Route B non-Review-Gate only when all escalation conditions are confirmed false). Section 11.7: companion file paths updated to v05 versions. Acceptance criterion 6 updated (v05 paths); acceptance criterion 15 added (Route B without escalation exemption). |

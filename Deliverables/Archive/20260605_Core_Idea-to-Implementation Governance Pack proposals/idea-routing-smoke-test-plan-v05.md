...
**Proposal status:** Draft v05
**Prepared by:** Larry (Team Orchestrator)
**Date:** 2026-06-05
**For:** Owner Walter Kamer
**Tests:** idea-routing-gl-proposal-v05.md, idea-routing-sop-proposal-v05.md, review-context-packet-sop-016-amendment-proposal-v05.md
...
## Revision Summary (v04 to v05)

The following changes were made in response to the Owner's v04 amendment request:

1. **Route Definitions section updated — Route B RCP exemption** — The Route B row and the Route B escalation conditions note are updated to state explicitly that Route B without escalation does not require an RCP unless the Owner explicitly requests one. This is consistent with the SOP-016 RCP amendment Section 11.2 v05 correction.
2. **Owner Decision Points section updated — Route B RCP note** — The Route B without escalation row is updated to state: no Review Gate required, no RCP required unless Owner explicitly requests one.
3. **Pass/Fail Summary updated** — Added criterion confirming that Route B without escalation does not require an RCP (unless Owner explicitly requests one). Added criterion confirming that if any Route B escalation condition is true, the route escalates and the Review Gate / RCP rules of the escalated route apply.
4. **Pending Proposals table updated** — All four file paths updated from v04 to v05 versions.
5. **All SOP-018 version references updated** — "SOP-018 v04" references in Test Assumptions section headers updated to "SOP-018 v05."
6. **Acceptance criterion 3 updated** — Now reads "SOP-018 v05" instead of "SOP-018 v04."
7. **Acceptance criterion 13 updated** — Now also verifies the Route B RCP exemption is stated in Test Assumptions and Pass/Fail Summary.
8. **Acceptance criterion 14 added** — Verifies that the Pass/Fail Summary includes explicit criteria for both the Route B RCP exemption and the escalated-Route-B behavior (Review Gate and RCP apply to the escalated route).
...
The following documents are pending approval. For smoke test purposes, use the v05 proposal content as the governance definition under test.

| Document | Path |
|---|---|
| GL-018 proposal v05 | `Deliverables/20260605_Core_Idea-to-Implementation Governance Pack proposals/idea-routing-gl-proposal-v05.md` |
| SOP-018 proposal v05 | `Deliverables/20260605_Core_Idea-to-Implementation Governance Pack proposals/idea-routing-sop-proposal-v05.md` |
| SOP-016 RCP amendment v05 | `Deliverables/20260605_Core_Idea-to-Implementation Governance Pack proposals/review-context-packet-sop-016-amendment-proposal-v05.md` |
| Smoke test plan v05 (this document) | `Deliverables/20260605_Core_Idea-to-Implementation Governance Pack proposals/idea-routing-smoke-test-plan-v05.md` |

### File Category Definitions (from SOP-018 v05 Section 2.4)
...
### Route Definitions (from SOP-018 v05 Section 9)
...
**Route B escalation conditions (from SOP-018 v05 Section 9):**
...
**Route B scope:** Route B applies to Low-impact ideas only. Route B is not available for Medium, High, or Critical impact. For S1-High scenarios, the route is Route C (full proposal required), not Route B. The Route Selection Matrix in SOP-018 v05 does not assign Route B to any Medium, High, or Critical cell.

**Route B RCP exemption:** Route B without escalation does not require a Review Gate and does not require an RCP unless the Owner explicitly requests one. This is per SOP-016 RCP amendment Section 11.2 v05: the RCP requirement applies to routes that require Review Gate under the active idea routing procedure. When any Route B escalation condition is true, the route escalates and the Review Gate and RCP rules of the escalated route apply in full.

### Owner Decision Points (from SOP-018 v05 Section 12)
...
**Route A:** Only DP-4.
**Route B without escalation:** DP-1, DP-3 (direct review — Owner reviews lightweight proposal directly, no Review Gate package), DP-4 (separate from DP-3). No Review Gate required. No RCP required unless Owner explicitly requests one.
...
### Review Gate Modes (from SOP-018 v05 Section 2.3 and SOP-016)
...
### Role-Based Definitions (from SOP-018 v05 Section 2)
...
| Route B without escalation: no RCP | Route B without escalation does not require an RCP unless the Owner explicitly requests one (per SOP-016 RCP amendment Section 11.2 v05) |
| Route B escalation triggers RCP | When any Route B escalation condition is true, the route escalates to Route C, D, or F; the Review Gate and RCP rules of the escalated route apply in full |
...
3. All test cases use DP-1 through DP-6 per SOP-018 v05; no test case references a DP outside this sequence.
...
13. The Route B scope note in the Test Assumptions section explicitly states that Route B applies to Low-impact ideas only, that S1-High routes to Route C, and that Route B without escalation does not require an RCP unless the Owner explicitly requests one. The Pass/Fail Summary includes a criterion confirming S1-High routing to Route C, confirming DP-4 is always a separate step from DP-3, and confirming the Route B RCP exemption.
14. The Pass/Fail Summary includes explicit criteria confirming: (a) Route B without escalation does not require an RCP unless Owner explicitly requests one, consistent with SOP-016 RCP amendment Section 11.2 v05; and (b) when any Route B escalation condition is true, the route escalates and the Review Gate and RCP rules of the escalated route apply in full. These two criteria together confirm both the exemption and its limit.
...
| Request amendments | Specific changes required; revised v06 prepared |
| Approve with modifications | Owner states modifications; v06 created before smoke test is considered for execution |
...
| 2026-06-05 | v05 | Route Definitions: added Route B RCP exemption note (Route B without escalation does not require an RCP unless Owner explicitly requests one; when escalation applies, escalated route RCP rules apply). Owner Decision Points: Route B without escalation row updated to state no Review Gate required and no RCP required unless Owner explicitly requests one. Pass/Fail Summary: added "Route B without escalation: no RCP" criterion and "Route B escalation triggers RCP" criterion. All SOP-018 version references updated from "v04" to "v05." Pending Proposals table: all paths updated to v05 versions. Acceptance criterion 3 updated (SOP-018 v05). Acceptance criterion 13 updated to also verify Route B RCP exemption in Test Assumptions and Pass/Fail Summary. Acceptance criterion 14 added (Pass/Fail Summary confirms both the Route B RCP exemption and the escalated-Route-B behavior). |

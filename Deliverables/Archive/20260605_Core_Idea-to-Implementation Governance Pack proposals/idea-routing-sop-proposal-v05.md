...
1. **Section 9 Route B steps corrected — DP-4 made explicit** — The Route B steps column now reads: "Short proposal → Owner direct review → Owner DP-3 decision → Implementation confirmation (DP-4) → Execution." The v03 text ("Owner decision → Execution") omitted DP-4 from the sequence, which could be read as DP-3 acceptance authorizing execution. DP-4 is a separate required step even for Route B without escalation.
2. **Section 9 Route B scope note added — Low-impact only** — A scope note has been added after the Route B escalation rule stating explicitly that Route B applies to Low-impact ideas only and is not available for Medium, High, or Critical impact.
3. **Section 8 Medium impact row updated** — "Route B or C required" changed to "Route C required; Route B does not apply at Medium or higher impact." This aligns Section 8 with the Route B scope restriction.
4. **Section 10 Route Selection Matrix corrected — S1-High** — S1-High changed from Route B* to Route C. High impact requires a full proposal, Review Gate, implementation report, and lifecycle decision. Route B does not satisfy these requirements and is not available at High impact.
5. **Section 13 Route B trigger line clarified** — "Route B without any of conditions 1a, 1b, 2–6" replaced with "Route B without any Route B escalation condition from Section 9." The Maintainer must explicitly confirm all Route B escalation conditions from Section 9 are false before treating Route B as non-Review-Gate. This removes the ambiguity of the previous cross-reference.
6. **Section 17 lifecycle timing clarified** — Section 17 now explicitly states that the implementation-route lifecycle decision occurs at DP-6 after DP-5 acceptance. Other accepted deliverables produced during idea routing (proposals, research briefs, review reports, closure reports) may separately receive SOP-017 lifecycle processing when accepted as standalone deliverables. DP-6 covers the implementation route only. Lifecycle processing requires explicit Owner approval.
7. **Acceptance criteria 16, 17, 18, and 19 added** — Verify Route B DP-4 explicitness, Route B Low-impact restriction, S1-High routing to Route C, and lifecycle timing consistency with GL-018 P8.
...
1. **EX-5 wording corrected — Route A vs. Route B for Low-impact ideas** — EX-5 previously stated "Low-impact ideas may proceed to Route A with Owner confirmation at DP-4 only." This could be read as requiring all Low-impact ideas to use Route A, excluding Route B. The corrected text reads: "Low-impact direct-execution ideas may proceed to Route A with Owner confirmation at DP-4 only. Low-impact ideas requiring a lightweight proposal follow Route B, subject to the Route B escalation conditions in Section 9." This does not weaken the requirement that Medium, High, and Critical ideas must be presented to the Owner at DP-1. The Route Selection Matrix (Section 10) is unchanged.
2. **Acceptance criterion 20 added** — Verifies EX-5 correctly distinguishes Route A (direct execution, Low-impact) from Route B (lightweight proposal, Low-impact) without conflating them or excluding Route B from Low-impact applicability.
...
16. Route B DP-4 is explicit in the Route Definitions table (Section 9): Route B steps read "Short proposal → Owner direct review → Owner DP-3 decision → Implementation confirmation (DP-4) → Execution." No Route B description implies DP-3 acceptance authorizes execution. The Route B scope note and the Section 12 Route B description both confirm DP-4 as a separate required step.
17. Route B is restricted to Low-impact ideas only. The Route B scope note in Section 9, the Route B escalation rule footnote in Section 10, and the Route Selection Matrix (Section 10) are consistent: Route B is not assigned to any Medium, High, or Critical impact scenario. S1-High routes to Route C, not Route B.
18. Section 8 (Impact Labels) Medium row states "Route C required" and explicitly states that Route B does not apply at Medium or higher impact. Sections 8 and 10 are internally consistent on the impact-to-route mapping.
19. Section 17 (Lifecycle Decision Requirements) is consistent with GL-018 P8: implementation-route lifecycle decisions occur at DP-6 after DP-5 acceptance; other accepted deliverables produced during idea routing may separately receive SOP-017 lifecycle processing as standalone deliverables; DP-6 covers implementation-route lifecycle only; lifecycle processing requires explicit Owner approval.
20. EX-5 correctly distinguishes Route A (direct execution, Low-impact only) from Route B (lightweight proposal, Low-impact) without conflating them or excluding Route B from Low-impact applicability. EX-5 states that Low-impact direct-execution ideas use Route A (DP-4 only), and that Low-impact ideas requiring a lightweight proposal follow Route B subject to Section 9 escalation conditions. EX-5 does not weaken the requirement that Medium, High, and Critical ideas must be presented to the Owner at DP-1.
...
| Request amendments | Specific changes required; revised v06 prepared; SOP-018 not created until approved |
| Approve with modifications | Owner states exact modifications; v06 created before SOP-018 is written |

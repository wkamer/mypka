...
13. **Section 22 added** — Pack-level Implementation Order and post-check sequence.
...
| Prohibited automatic actions | Modifying any script or system file before proposal is reviewed and Owner decision at DP-3 is made. Skipping Review Gate for system-file modifications. |
...
| Required Owner decision | DP-1 (route confirmation); DP-2 (research scope); DP-3 (post-Review Gate); DP-4 (implementation confirmation) |
...
| Prohibited automatic actions | Beginning implementation before research brief is delivered and DP-3 Owner decision is made. Installing external tools or services without Owner approval. |
...
| Review Gate applies | Yes — always; governance document changes are governance-critical; Mode 1 preferred |
| Implementation report applies | Yes |
| Lifecycle decision applies | Yes |
...
| Prohibited automatic actions | Modifying any GL, SOP, Workstream, AGENT.md, or CLAUDE.md before SOP-015 proposal is accepted at DP-3. Treating a verbal agreement as an approved proposal. Applying exact-text check without the exact proposed text being in the accepted proposal. |
...
| **Medium** | Requires script modification, new configuration, or operational procedure change; no governance document changes; reversible with documented steps | Route B or C required; proposal required; Review Gate required if any Review Gate trigger in Section 13 applies |
...
1. No system file is created, modified, or deleted (system files: any GL, SOP, Workstream, AGENT.md, CLAUDE.md, or file listed in the active canonical paths reference)
...
|---|---|
| Condition 1, 2, 5, or 6 | Escalate to Route C |
| Condition 3 (governance impact) | Escalate to Route D |
...
| DP-2 | Research brief scope ready | Approve research scope before research begins | Route E only |
...
**Route E additionally:** DP-2 (research scope) before DP-3.
...
2. The deliverable modifies any system file: AGENT.md, SOP, GL, Workstream, CLAUDE.md, or any file in the active canonical paths reference
...
Route B without any of conditions 1–6: Review Gate is not required. The Maintainer must confirm in writing that all six conditions are false before treating it as Route B without Review Gate.
...
**EX-2:** No system file (GL, SOP, Workstream, AGENT.md, CLAUDE.md) may be modified as part of any route except S8/Route D, and only after SOP-015 approval at DP-3.
...
**HR-7:** Route B may not claim to be free of Review Gate triggers without the Maintainer explicitly confirming in writing that all six Route B escalation conditions (Section 9) are false.
...
4. The Route B escalation rule (Section 9) is complete: all six conditions are stated and the escalation path for each is defined.
...
12. All 7 explicit exclusions (EX-1 through EX-9) and 7 hard rules (HR-1 through HR-7) are internally consistent and do not conflict with GL-014, GL-015, GL-016, GL-017, SOP-015, SOP-016, or SOP-017.
13. The proposed SOP number SOP-018 is proposal-only; re-confirmation note is present.
14. No execution has occurred as a result of this proposal document.
...
| Request amendments | Specific changes required; revised v03 prepared; SOP-018 not created until approved |
| Approve with modifications | Owner states exact modifications; v03 created before SOP-018 is written |

...
**Supersedes:** `20260613_archive-write-plan-ids-2-3-v01.md`
**Correction v02:** W-3, W-4, and V-3 updated to include `owner_decision = 'archive'`.
...
cur = conn.cursor()
cur.execute(
    "UPDATE deliverable_lifecycle SET state_gl017 = 'archived', state_changed_at = datetime('now'), owner_decision = 'archive' WHERE id = 2"
)
conn.commit()
...
cur = conn.cursor()
cur.execute(
    "UPDATE deliverable_lifecycle SET state_gl017 = 'archived', state_changed_at = datetime('now'), owner_decision = 'archive' WHERE id = 3"
)
conn.commit()
print(f"Rows updated: {cur.rowcount}")
conn.close()
...
```sql
SELECT id, artifact_name, state_gl017, state_changed_at, owner_decision FROM deliverable_lifecycle WHERE id IN (2, 3);
```
Expected: both rows show `state_gl017 = 'archived'`, non-null `state_changed_at`, and `owner_decision = 'archive'`.
...
Delivered at: Deliverables/20260612_Core_DL Control Inventory/20260613_archive-write-plan-ids-2-3-v02.md

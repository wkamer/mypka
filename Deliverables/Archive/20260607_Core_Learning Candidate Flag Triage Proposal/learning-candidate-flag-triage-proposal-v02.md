# Learning Candidate Flag — Triage Proposal v02

**Document type:** Read-only triage proposal
**Date:** 2026-06-07
**Author:** Larry (Team Orchestrator)
**Status:** Awaiting Owner decision
**Supersedes:** `learning-candidate-flag-triage-proposal-v01.md`
...
The CLAUDE.md governance rule says "invoke the Gatekeeper at the relevant checkpoint (CP-1 through CP-4) per SOP-019" but no SOP-019 file exists at `Team Knowledge/Core/SOPs/`. The rule references a document that is absent. This is flagged as a separate observation and is not part of this triage decision.
...
Additionally, SOP-019 does not exist as a file. The action would be to create SOP-019, not to correct it. Creating a new SOP requires the governance rule to be established first through Owner decision.

### Why Level 2

The observation qualifies as Level 2 because:
- It requires Owner input before any behavioral rule is established.
- The gap is systemic (all checkpoints missed) but not immediately structurally correctable without Owner input on governance intent.
...
The route is split into two separate decisions. This triage covers Decision 1 only. The Owner decides on Decision 2 after Decision 1 is complete.

### Decision 1 — Triage classification (Owner action now)

1. Owner confirms triage: register as Learning Candidate (Level 2).
2. Larry prepares a registration proposal. Iris reviews. Owner authorizes the registration separately.
3. Larry registers the Learning Candidate in the `learning_candidates` table (team-knowledge.db), status set to `surfaced`.
4. Larry surfaces the Learning Candidate to the Owner in this session (CAT-3 rule: same session as flagging, no delay).
5. team_tasks id 73 remains open until registration and surfacing are both complete.
...
- Larry initiates SOP-019 creation as a CAT-3 structural action.
- Larry prepares a structural proposal. Iris reviews. Owner authorizes. Kai implements if database work is involved. Nolan updates AGENT.md files if agent behavior rules are changed.
- team_tasks id 73 is closed after registration and surfacing are confirmed complete, independent of the SOP-019 route.

---

## 6. Whether Iris Review Is Required Before Any Implementation

**For this triage proposal:** No. A triage proposal is read-only. It is Larry's advisory work. No write actions are involved.
...
1. Larry prepares a complete proposal (this document covers the triage; a registration proposal follows separately).
2. Iris reviews the registration proposal.
3. Larry presents the reviewed proposal to the Owner.
4. Owner confirms.
5. Larry executes.

Iris has not reviewed the Learning Candidate registration proposal yet. That is a separate next step after Owner confirms this triage.
...
|---|---|
| `team-knowledge.db` table `learning_candidates` | Learning Candidate row INSERT — after Owner separately confirms registration |
| `team-knowledge.db` table `team_tasks` | task id 73: status set to completed — after registration and surfacing are confirmed complete |
| `Team Knowledge/Core/SOPs/SOP-019_Governance Gatekeeper SOP.md` | To be created if Owner promotes — this file does not exist yet |
...
- No write actions of any kind until Owner confirms this triage and separately confirms each subsequent step.

---

## 9. Can the Owner Answer with Yes, No, or Correction?

Yes. This proposal presents one recommendation. The Owner does not need to elaborate to proceed.

- **Yes:** Learning Candidate (Level 2) is the correct classification. Larry proceeds to prepare a registration proposal for Iris review. team_tasks id 73 stays open. Promotion to SOP-019 is not yet decided.
- **No:** The recommended classification is rejected. Larry asks for the preferred alternative before any action.
- **Correction:** The Owner adjusts the classification. Larry incorporates and confirms before proceeding.

---

## 10. Exact Next Owner Action

> **Confirm the triage classification:**
>
> 1. Register this as a Learning Candidate (Level 2).
> 2. Surface it in the same session because it is CAT-3.
> 3. Keep team_tasks id 73 open until registration and surfacing are complete.
>
> Promotion to SOP-019 is the recommended likely next decision after registration and surfacing. It is not part of this confirmation.
>

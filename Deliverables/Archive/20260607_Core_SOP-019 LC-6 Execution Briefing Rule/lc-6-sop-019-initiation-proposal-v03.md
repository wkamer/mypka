...
Iris accepted v02 (2026-06-07) with one edge-case flag: the proposed rule text had no explicit handling for the case where Larry prepares an execution brief without an associated write-list. The violation trigger was therefore undefined in that scenario — Larry could either silently skip the check or incorrectly halt.

v03 resolves this by restructuring the rule text into two explicit cases: (1) write-list present — include all batch-stop rules; (2) no write-list — explicitly state this in the brief. Silent omission is not permitted in either case. The violation trigger has been updated to cover both cases.

Additional updates: Section 7 risk register (No write-list present row), Section 10 Iris review prompt (path and context), Section 11 correction history.

No other content changes. Scope, destination, write-plan, and post-check requirements are unchanged from v02.

---

**v02 superseded v01** because v01 contained Dutch wording in proposed system-facing and governance-facing content.
...
The following text is proposed for insertion into `CLAUDE.md`, under `## Hard Rules`, as a new subsection titled `### Execution Briefing — Batch-Stop Rules (mandatory)`:
...
**Not a SOP:** SOPs define multi-step atomic procedures for recurring workflows. This rule is a single-sentence behavioral requirement embedded in briefing, not a standalone procedure. There is no sequence of steps to follow — there is one obligation: address batch-stop rules in the brief. A dedicated SOP would add structural weight to a rule that belongs as a standing operational constraint.
...
| No write-list present | Some execution briefs may not have an associated write-list | Rule explicitly handles this case: Larry must state in the brief that no write-list exists and therefore no write-list batch-stop rules apply. The check is not skipped — it is satisfied by explicit declaration. Silent omission remains a violation. |
...
| Friction in fast-moving sessions | Rule adds a briefing step to every execution brief, potentially slowing sessions | The check is minimal. When no write-list exists, the rule is satisfied by one declarative sentence. When a write-list exists with no batch-stop rules, the rule adds zero friction. |
| Ambiguity on what counts as a batch-stop rule | Executors and Larry may disagree on what is a batch-stop rule vs. a general instruction | Rule text provides a definition. If ambiguity arises, the default is to include the rule in the brief rather than exclude it. |
...
|---|---|---|
| `CLAUDE.md` | Insert new subsection `### Execution Briefing — Batch-Stop Rules (mandatory)` under `## Hard Rules`, after "Domain Check Before Execution — verplicht" | Exact text defined in Section 5 of this proposal |
...
> Iris, please review the CLAUDE.md amendment proposal at `Deliverables/20260607_Core_SOP-019 LC-6 Execution Briefing Rule/lc-6-sop-019-initiation-proposal-v03.md`. This is v03 of the proposal. v02 was accepted by Iris with one edge-case flag: the rule text had no explicit handling for execution briefs without an associated write-list. v03 resolves this by restructuring the rule into two explicit cases and updating the violation trigger to cover both. The proposal adds a new Hard Rule subsection titled "Execution Briefing — Batch-Stop Rules (mandatory)" to CLAUDE.md, inserted after the existing "Domain Check Before Execution" subsection. Review for: (1) alignment with GL-020 intent classification, (2) conflict with existing Hard Rules, especially Domain Check Before Execution, (3) whether the edge-case flag from v02 is adequately resolved, (4) unintended scope expansion, (5) execution risk. This is advisory review only. Do not authorize. Return your findings for Larry to present to the Owner.
...
**Correction history:**
- v01: Dutch wording in proposed system-facing text. Superseded by v02.
- v02: Iris accepted; edge-case flagged (no handling for execution briefs without a write-list). Superseded by v03.
- v03: Edge-case resolved in rule text. Awaiting Iris review and Owner authorization.
...
- **Yes** — proceed with execution as proposed
- **No** — do not proceed; proposal is closed
- **Correction** — specify what to change; Larry creates v04 and Iris reviews again before re-presenting
...
|---|---|
| Rule present in Hard Rules | The new subsection `### Execution Briefing — Batch-Stop Rules (mandatory)` appears under `## Hard Rules` in CLAUDE.md |
| Correct placement | New subsection is immediately after "Domain Check Before Execution — verplicht" |
| Text matches approval | Rule text matches the approved version from this proposal (or v04+ if a correction was applied) exactly |
...
This proposal is a read-only revision document.
...
`Deliverables/20260607_Core_SOP-019 LC-6 Execution Briefing Rule/lc-6-sop-019-initiation-proposal-v03.md`

v01 and v02 remain in the same folder as version history records and are not deleted.

---

## 14. Next Step Recommendation

**Recommended next step: Iris review of this proposal (v03).**

This proposal is complete and corrected. The next action is to invoke Iris review using the exact prompt in Section 10. After Iris returns findings, Larry presents the reviewed proposal to the Owner for authorization.
...
*Prepared: 2026-06-07*
*Revised: 2026-06-07 (v02 — Dutch wording removed from system-facing and governance-facing content)*
*Revised: 2026-06-07 (v03 — edge-case for no-write-list execution briefs resolved in rule text; risk register, Iris prompt, and correction history updated)*
*Delivered on: 2026-06-07*
*Delivered at: Larry, read-only revision step*

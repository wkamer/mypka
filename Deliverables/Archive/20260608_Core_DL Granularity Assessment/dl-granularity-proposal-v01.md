# Deliverable Granularity Rules — Implementation Proposal v01

**Date:** 2026-06-08
**Prepared by:** Larry
**Type:** Implementation proposal — not an assessment
**Status:** Awaiting Owner approval and Iris review gate (GL-016 / SOP-016)
**Basis:** Deliverable Lifecycle Granularity Assessment (2026-06-08), accepted by Owner
**Parent folder:** `20260608_Core_DL Granularity Assessment/` — this file is a phase-2
supporting artifact within the Granularity Assessment initiative, not a standalone folder

**Note — self-referential demonstration:**
This proposal is itself placed as a file inside the Granularity Assessment folder rather
than in a new standalone folder. This is the correct behavior under the rules defined
below. The placement demonstrates the rule in effect.
...
- `session-start-verification-lcl-v01.md` → session log or LC triage deliverable folder — Rule G2-E
- `session-start-verification-post-sop019-v01.md` → session log or SOP-019 deliverable folder — Rule G2-E
...
**Under proposed rules:**
This proposal is placed as `dl-granularity-proposal-v01.md` inside
`20260608_Core_DL Granularity Assessment/` — Rule G2-F.

The Granularity Assessment is the primary initiative deliverable. The proposal is a
phase-2 output of that same initiative. It does not yet satisfy G1-E (no Iris review
completed) or G1-B (Owner has not yet approved it as a standalone artifact). Once
approved and Iris-reviewed, it may satisfy G1-E and upgrade to a primary deliverable —
but only at that point, and only if the Owner or team needs to cite it independently.
...
**Target file:** `Team Knowledge/Core/SOPs/SOP-019_[filename].md`

**Note:** SOP-019 governs the Governance Gatekeeper procedure. The amendment adds a
placement rule for track-level artifacts. The exact section number requires reading the
current SOP-019 structure before writing. The amendment text is specified here; section
placement is confirmed at write time.
...
These steps are executed only after Owner approval of this proposal and after the Iris
review gate (GL-016 / SOP-016) passes. Steps are sequential. Each step is gated by the
batch-stop rules in Section 8.

| Step | Action | Actor | Target file |
|---|---|---|---|
| 1 | Owner approval of this proposal | Owner | This file |
| 2 | Iris review gate (GL-016 / SOP-016) | Iris (via Larry) | This proposal |
| 3 | Write GL-017 amendment (Sections 2.1 and 2.2) | Larry or Kai | `GL-017_Deliverable Lifecycle Knowledge Processing and Archiving.md` |
| 4 | Write SOP-017 amendment (Section 16 addition + new Section 4a) | Larry or Kai | `SOP-017_Deliverable Lifecycle Knowledge Processing and Archiving Procedure.md` |
| 5 | Read SOP-019 structure; confirm section placement; write SOP-019 amendment | Larry or Kai | `SOP-019_[filename].md` |
...
| BS-2: GL-017 amendment conflicts with existing G-017 principles P1–P13 | Post-check P6 fails |
...
serve the function of a write-list, specifying each target file and action. A separate
write-list document is not warranted (it would be a G2-D artifact that belongs here,
not in a new folder — consistent with the rules this proposal defines).

There is therefore no write-list batch-stop inheritance concern. The batch-stop rules
in Section 8 are the complete and authoritative batch-stop set for this proposal's
implementation.

---

Delivered on: 2026-06-08
Delivered at: Deliverables/20260608_Core_DL Granularity Assessment/dl-granularity-proposal-v01.md

# LC Batch 2 Write-List — v04

**Date:** 2026-06-07
**Author:** Larry
**Status:** Pending Iris review — not yet authorized for execution
**Supersedes:** `lc-batch2-write-list-v03.md`
...
**Corrections in v04 vs v03:**
- `resolve_lc` wrapped in explicit `BEGIN / COMMIT / ROLLBACK` transaction — all three branches (apply, reject, escalate) protected
- Reject branch still follows `processed → closed` per GL-022 — both UPDATEs now run inside one atomic transaction, eliminating stranding risk
- Post-checks strengthened: verify transaction presence, verify commit after branch logic, verify rollback on exception, verify reject ordering

---

## Scope and Dependencies

Batch 2 aligns the operational /close-session skill to the model established in Batch 1. GL-021 is in scope as a finding only — no write required (see W-5).
...
**Batch stop rule:** If any W-4 sub-edit fails or produces unexpected output, stop immediately and report before applying any remaining sub-edits.

---

## W-4 — /close-session skill update

**Purpose:** Align the three LC-related sections of /close-session to the new schema: Step 1 LC scan (status values and column names), Step 1b write plan label, and Step 3b LC sweep (SQL, Python logic, decision verbs, and transaction-wrapped resolve_lc function).
...
def resolve_lc(lc_id, decision):
    ...
    try:
        ...
        if decision == 'apply':
            ...
            # Two-step transition per GL-022: processed → closed.
            # Both steps inside one transaction — no stranding risk.
            conn.execute(
                """UPDATE learning_candidates
                   SET status='processed', processed_at=datetime('now'),
        ...
        conn.rollback()
        print(f"LC-{lc_id} resolve FAILED: {e}")
    finally:
        conn.close()
...
For `apply`: LC transitions to `processed` with `processed_outcome = 'team_learning'` (or `agent_learning` for agent-scoped LCs). All changes committed atomically.

For `reject`: LC transitions through `processed` (with `processed_outcome = 'rejected'`) then immediately to `closed`, both inside one transaction — no stranding risk.
...
checks = [
    ...
    ('Step 3b UPDATE uses triaged status',       \"status = 'triaged'\" in content),
...
func_m = re.search(r'def resolve_lc\(.*?finally:\s*conn\.close\(\)', content, re.DOTALL)
if func_m:
    fn = func_m.group()
    checks += [
        ('resolve_lc contains explicit BEGIN',        'conn.execute(\"BEGIN\")' in fn),
        ('resolve_lc contains commit after branches', 'conn.commit()' in fn),
        ('resolve_lc contains rollback on exception', 'conn.rollback()' in fn),
        ('resolve_lc closes in finally block',        'finally' in fn and 'conn.close()' in fn),
    ]
else:
    checks.append(('resolve_lc function found', False))

# Escalate branch checks
escalate_m = re.search(r\"elif decision == 'escalate':(.*?)conn\\.commit\(\)\", content, re.DOTALL)
if escalate_m:
    ...
    checks.append(('escalate branch found', False))

# Reject branch checks
reject_m = re.search(r\"elif decision == 'reject':(.*?)elif decision == 'escalate'\", content, re.DOTALL)
if reject_m:
    rb = reject_m.group(1)
    checks += [
        ('reject sets status=processed first',              \"status='processed'\" in rb),
        ('reject sets processed_outcome=rejected',          \"processed_outcome='rejected'\" in rb),
        ('reject sets status=closed after processed',       \"status='closed'\" in rb),
        ('reject processed precedes closed (ordering)',      rb.index(\"status='processed'\") < rb.index(\"status='closed'\")),
    ]
else:
    checks.append(('reject branch found', False))
...
**Finding:** The text "UPDATE status → 'surfaced'" and "UPDATE status → approved / rejected / promoted" referenced in impact assessment items 30–31 does not exist in GL-021. These strings were in GL-022 Section 7, corrected in Batch 1 W-2. GL-021 Section 7 contains only general rules about pre-authorized writes with no specific LC status values.

**Action:** No change to GL-021 required in Batch 2. Finding documented.

**W-5 status:** No write action.
...
| W-4c | /close-session Step 3b: SQL, Python, decision verbs, transaction-wrapped `resolve_lc` (apply/reject/escalate) per GL-022 |
...
Delivered at: `Deliverables/20260607_Core_LC Batch 2 Write-List/lc-batch2-write-list-v04.md`

...
- Domain: Personal (PKM)
- Location: `Team/Iris - The Journal Agent/`
...
1. **Journal entry.** Write the entry as a Markdown file at `PKM/Journal/YYYY/MM/YYYY-MM-DD-<slug>.md` with front-matter (`date`, `title`, `entry_type`). Insert one row into `PKM/personal.db` table `journal`.
...
5. **Image OCR.** OCR any attached image. File to `PKM/Images/YYYY/MM/`. Create a `documents` row. Link via `document_mentions`.

6. **Document filing.** File any PDF or scan to `PKM/Documents/` using SOP-009 naming (`YYYYMMDD_Type_Person_Provider_Detail.ext`). Create a `documents` row with metadata.

7. **Session log.** Write one `session_log` row to `Team Knowledge/team-knowledge.db` table `session_logs` with `agent_slug="journal-agent"` after each processing run.
...
- Core database: `Team Knowledge/team-knowledge.db`
- PKM index: `PKM/pkm-index.md`
- Journal folder: `PKM/Journal/`
- Images folder: `PKM/Images/`
- Documents folder: `PKM/Documents/`
- SOP-009: `Team Knowledge/Personal/SOPs/SOP-009-document-naming.md`

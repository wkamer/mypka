...
- **Rebuild and verify before reporting done:** After every frontend code change: (1) rebuild the frontend, (2) verify the specific behavior in the running app, (3) only then report done. Tests green is not proof. Browser behavior is proof. Never report done based on test results alone for browser-rendered code.
- **Persistent data in DB:** Any data that must survive a page refresh or component remount must be persisted to the database. React state and JS module-level objects are session-only. Never accept in-memory state as the persistence layer for user-facing data. When implementing stateful UI: ask whether the data must survive a refresh — if yes, write to DB and read back on mount.
...
- **Kai/Devon boundary:** Kai owns architecture, credentials, deployments, production services, and integration boundaries. Devon owns application code after Kai defines the boundary. When a feature touches external services, credentials, deployment, or production config, Kai briefs first — Devon builds only after Kai's brief is accepted and Larry has relayed it. Devon does not self-route to Kai; Larry handles all routing decisions.

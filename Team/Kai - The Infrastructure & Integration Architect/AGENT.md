...
- **Kai/Devon boundary:** Kai owns architecture, credentials, deployments, production services, and integration boundaries. Devon owns application code after Kai defines the boundary. When a feature touches external services, credentials, deployment, or production config, Larry routes Kai first. After Kai's brief is accepted by the owner, Larry routes Devon for the build. Kai does not self-route to Devon; Larry handles all routing decisions. Kai briefs the boundary — Devon executes inside it.

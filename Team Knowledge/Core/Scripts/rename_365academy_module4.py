"""
rename_365academy_module4.py

Renames all lesson files in the Module 4. Doelloosheid folder to:
    Les <nr>. <title>.<ext>
...
Usage:
    python rename_365academy_module4.py [--execute]
...
FOLDER = "/opt/myPKA/Team Inbox/4. Doelloosheid"
SKIP_FILES = {
    "Introductie - Dynamische Doelloosheid vrede met je toekomst.txt",
}
...
def build_rename_map(folder: str) -> list[tuple[str, str]]:
    ...
    lesson_txt_re = re.compile(r"^365-academy - 4\.(\d+) .+\.txt$")
    jpg_re = re.compile(r"^.+-miracle-roadmap[-_]les-(\d+)\.jpg$")
    media_re = re.compile(r"^365-academy - 4\.(\d+) .+\.(mp4|srt|txt)$")

    # Extract titles from .txt files
...
def print_preview(renames: list[tuple[str, str]]) -> None:
    ...
    print(f"\n{'PREVIEW — 365 Academy Module 4 rename':^80}")
    print("=" * 80)

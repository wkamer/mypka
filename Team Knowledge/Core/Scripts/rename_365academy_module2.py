"""
rename_365academy_module2.py

Renames all lesson files in the Module 2. Radicale Aanvaarding folder to:
    Les <nr>. <titel>.<ext>
...
Usage:
    python rename_365academy_module2.py [--execute]
...
FOLDER = "/opt/myPKA/Team Inbox/2. Radicale Aanvaarding"
SKIP_FILES = {
    "Introductie - Radicale aanvaarding vrede met je verleden.txt",
    "365-academy - 2.35 Wie je werkelijk bent is onbegrensd365-academy - 2.36 In waarheid zijn we altijd verbonden.srt",
}
...
def build_rename_map(folder: str) -> list[tuple[str, str]]:
    ...
    lesson_txt_re = re.compile(r"^365-academy - 2\.(\d+) .+\.txt$")
    jpg_re = re.compile(r"^.+-miracle-roadmap_les-(\d+)\.jpg$")
    media_re = re.compile(r"^365-academy - 2\.(\d+) .+\.(mp4|srt|txt)$")
...
def print_preview(renames: list[tuple[str, str]]) -> None:
    ...
    print(f"\n{'PREVIEW — 365 Academy Module 2 rename':^80}")
    print("=" * 80)
...
def main():
    parser = argparse.ArgumentParser(description="Rename 365 Academy Module 2 lesson files.")

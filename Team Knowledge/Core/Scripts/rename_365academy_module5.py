"""
rename_365academy_module5.py

Renames all lesson files in the Module 5. Eenheid folder to:
    Les <nr>. <title>.<ext>
...
Usage:
    python rename_365academy_module5.py [--execute]
...
FOLDER = "/opt/myPKA/Team Inbox/5. Eenheid"
SKIP_FILES = {
    "Introductie - Eenheid vrede met alles.txt",
}
...
def build_rename_map(folder: str) -> list[tuple[str, str]]:
    ...
    lesson_txt_re = re.compile(r"^365-academy - 5\.(\d+) .+\.txt$")
    jpg_re = re.compile(r"^.+-miracle-roadmap[-_]les-(\d+)\.jpg$")
    media_re = re.compile(r"^365-academy - 5\.(\d+) .+\.(mp4|srt|txt)$")
...
def print_preview(renames: list[tuple[str, str]]) -> None:
    ...
    print(f"\n{'PREVIEW — 365 Academy Module 5 rename':^80}")
    print("=" * 80)

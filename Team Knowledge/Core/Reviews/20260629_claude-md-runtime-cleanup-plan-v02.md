# CLAUDE.md Runtime Cleanup Plan — v02
...
**Supersedes:** v01 (`20260629_claude-md-runtime-cleanup-plan-v01.md`)
...
**v01 correction:** v01 stated "seven sections" but listed eight. Fixed here.
...
| Task Systems — Todoist project IDs | Task Systems | New file: `Team Knowledge/Core/Guidelines/GL-025_todoist-projects.md` | Not routing logic; reference data. **v01 correction: was GL-005, now GL-025.** |
...
| Project Creation 5-step process | Project Creation | New SOP: `Team Knowledge/SOPs/SOP-019_project-creation.md` | **v01 correction: was SOP-026, now SOP-019.** CLAUDE.md retains one-line pointer after removal: "Project creation → [[SOP-019_project-creation]]" |
| Naming Conventions table | Naming Conventions | **Conditional — see pre-execution check in Section 8:** either append to `Team Knowledge/Core/Guidelines/GL-001_File naming conventions.md`, or create new `Team Knowledge/Core/Guidelines/GL-026_naming-conventions.md` | GL-001 must be inspected before deciding. If GL-001 already covers the same convention types, append. If scope differs, create GL-026. **v01 correction: was GL-006, now GL-026 (if new file needed).** |
...
| Build routing explanatory paragraphs | Key Routing Rules | `Team/Devon - The Senior Full-Stack Developer/AGENT.md` + `Team/Kai - The Infrastructure & Integration Architect/AGENT.md` | Justifications for boundaries already set. **v01 correction: paths were wrong (Team Knowledge/...). Correct paths used here.** |
...
|------|----------------|--------|
| Full Changelog section (6 entries) | Changelog | Git history is the record. Each removal commit must reference team_task 113 and "Iris review 2026-06-29" in the commit message so the audit trail is unambiguous. **Iris pre-condition:** verify session logs capture any authorization events currently referenced only in the Changelog before removal. |
...
| 2 | `Team/agent-index.md` | Verify + complete | Must list all 20 specialists before team table is removed from CLAUDE.md |
...
| 5 | `Team Knowledge/Core/Guidelines/GL-001_File naming conventions.md` OR `GL-026_naming-conventions.md` | Append OR new file | Naming conventions table — GL-001 inspection determines which. **Conditional: pre-execution check required (see Check 11 in Section 8).** |
| 6 | `Team Knowledge/SOPs/SOP-019_project-creation.md` | New file | Project creation 5-step process |
| 7 | `Team/Larry - The Orchestrator/AGENT.md` | Append | Execution relay pattern, hiring rule, learning rule, task systems explanation, build routing context. **Pre-condition: verify file exists at this exact path (Check 12 in Section 8).** |
...
**Total: 10 files (file 5 is conditional — 1 or 2 files depending on GL-001 inspection).**
**3 new files (minimum), 7 updated files.**
...
Execution order is strict. No step may begin before its predecessor is complete. Each step is a separate git commit.

### Pre-execution checks (before any write)
...
**Check 3 — agent-index.md completeness:** Run `ls Team/` and compare against CLAUDE.md team table. All 20 specialists must appear in agent-index.md before team table removal.

**Check 4 — GL-004 path coverage:** Read GL-004. Confirm every path in CLAUDE.md "Where Things Live" is present in GL-004. Any gap must be filled as part of the GL-004 append (file 3) before CLAUDE.md section is removed.

**Check 5 — Changelog authorization audit (Iris pre-condition):** Read current CLAUDE.md Changelog section. For each entry referencing an Iris audit event or Owner confirmation, verify the corresponding session log records that authorization. If any authorization event exists only in the Changelog, add it to the relevant session log before deletion.

**Check 6 — SOP-index.md reserved entries:** Read `Team Knowledge/SOPs/SOP-index.md`. Confirm SOP-019 is not reserved or assigned as a draft entry. If SOP-019 is taken, use next free sequential number and update Section 7 accordingly.

**Check 11 — GL-001 inspection (naming conventions decision):** Read `Team Knowledge/Core/Guidelines/GL-001_File naming conventions.md`. If GL-001 covers entity-type naming conventions (P-, T-, KE-, G-, SOP-, GL- prefixes), append the CLAUDE.md naming table there. If GL-001 covers only file-level naming (case, separators, extension), create GL-026 as a separate file. Record the decision before executing.

**Check 12 — Larry AGENT.md path:** Run `ls "Team/Larry - The Orchestrator/"` to confirm AGENT.md exists at that exact path. If not found, pause and report to Owner before any append.

### Execution sequence (after all pre-execution checks pass)

**Step 1:** Add Hygiene Rule section to CLAUDE.md. Git commit: "feat(CLAUDE.md): add Hygiene Rule and Decision Boundary section — team_task 113, Iris review 2026-06-29"

**Step 2:** Append Deliverable rule to GL-004. Git commit.

**Step 3:** Create GL-025_todoist-projects.md. Git commit.

**Step 4:** Create GL-026 or append to GL-001 (per Check 11 decision). Git commit.
...
**Step 10:** Verify agent-index.md is complete (Check 3). If not complete, complete it now. Git commit.

**Step 11:** Rewrite CLAUDE.md (all removals, compressions, one-line Project Creation pointer added). Git commit: "refactor(CLAUDE.md): runtime-only rewrite — team_task 113, Iris review 2026-06-29"

**Hard rule:** Step 11 (CLAUDE.md rewrite) may not execute until Steps 1-10 are complete and confirmed. Steps 7 and 8 (Devon and Kai AGENT.md appends) must complete before Step 11 — not after.
...
| # | Iris answer | Resolution in v02 |
|---|-------------|------------------|
| 1 | Changelog removal is safe. Commit messages must reference team_task 113 and Iris review date. Verify session logs capture any authorization events first. | Check 5 added to pre-execution checks. Commit message format specified in Step 11. |
| 2 | Sequence is correct. Two risks: (a) Check 8 (Todoist grep) must run before Owner approval; (b) Devon and Kai AGENT.md appends missing from verification sequence. | Check 8 completed in discovery report. Devon and Kai appends added as Steps 7 and 8, with sequencing gate before Step 11. |
...
| # | Iris flag | Resolution in v02 |
|---|-----------|------------------|
| A | Devon and Kai AGENT.md paths wrong (`Team Knowledge/` instead of `Team/`) | Corrected in Section 3 and Section 7. |
| B | GL-005 and GL-006 numbers taken; SOP-026 not sequential | GL-025 and GL-026 used. SOP-019 used. Checks 6 and 11 added. |
...
| D | Larry AGENT.md path unverified | Check 12 added to pre-execution checks. |
...
| Check | Discovery result | v02 resolution |
|-------|-----------------|---------------|
| GL-005 free? | FAIL — GL-005 taken (AI Engineering OS) | Renumbered to GL-025. |
| GL-006 free? | FAIL — GL-006 taken (Notification Format) | Renumbered to GL-026 (conditional on GL-001 inspection — Check 11). |
...
| Sienna hardcoded ID | PRE-EXISTING NOTE — Sienna AGENT.md:256 has personal project ID (6cFcm2MpmHvc2F3H) | Treated as pre-existing SSOT note. Not created by this cleanup. Owner decision recorded in Section 11. |

---

## 11. Owner Decisions Recorded

The following decisions were made by the Owner during this review cycle and are binding for this execution.
...
Owner approval of v02 authorizes **only the following:**

- All writes listed in Section 7 (files 1-10), in the execution sequence defined in Section 8.
- No Sienna AGENT.md change.
- No additional files beyond Section 7 unless execution pauses for explicit Owner confirmation.
- If Check 6 (SOP-index.md) or Check 11 (GL-001 inspection) or Check 12 (Larry AGENT.md path) reveals a conflict or gap outside Section 7, execution pauses at that step. Larry reports the finding. Owner confirms expanded scope or provides a correction before execution resumes.
- If any pre-execution check fails, execution does not begin. Larry reports the failure and routes the correction for Owner confirmation.
...
*Review v02. Execution authorized upon Owner confirmation.*
